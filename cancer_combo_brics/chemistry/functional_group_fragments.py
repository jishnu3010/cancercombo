"""RDKit functional-group fragmentation module with context preservation.

Identifies functional groups in a molecule using RDKit SMARTS matching and extracts
each functional group along with its surrounding molecular/attachment context.
...
Context Preservation Strategy:
  ...
  - Extracts the sub-molecule for the expanded atom set using RDKit to preserve valid connectivity.
  - Converts sub-molecules into canonical SMILES fragment strings.

Fallback Handling:
  - If a molecule has no recognized functional group (or if parsing fails), the module returns
    the full canonical SMILES of the molecule as a single fragment.
  - No drug molecule is ever silently discarded.
"""
...
def extract_fragment_with_context(
    ...
    """Extract sub-molecule SMILES for a matched core plus its neighboring context.

    Args:
        mol: Parent RDKit Mol object.
        match_indices: Atom indices of matched functional group core.
        radius: Number of topological hops to expand context.

    Returns:
        Canonical fragment SMILES string with context, or None if extraction fails.
    """
    ...
    try:
        ...
        logger.warning(f"Error extracting fragment context for indices {match_indices}: {e}")
        return None
...
def extract_functional_group_fragments(
    ...
    # Fallback to full molecule if no functional groups detected
